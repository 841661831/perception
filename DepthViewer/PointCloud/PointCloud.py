...
# from second.sort_0608 import Sort
from second.sort_0610 import Sort
...
from second.all_track import rotate_box, fusion_box, get_min_max, fix_cluster, cal_box_lane, cal_map_angle, find_angle
import cv2
...
strExePath = getExcutePath()
# #####*****  广州隧道参数  *****#####
lane_with_angle_adr = strExePath + '/Configs/Lane/lane_with_angle.npy'
left_center_adr = strExePath + '/Configs/Lane/left_center.npy'
medium_center_adr = strExePath + '/Configs/Lane/medium_center.npy'
right_center_adr = strExePath + '/Configs/Lane/right_center.npy'
...
left_center = np.load(left_center_adr)
medium_center = np.load(medium_center_adr)
right_center = np.load(right_center_adr)
left_center = left_center[left_center[:, 0].argsort()]
medium_center = medium_center[medium_center[:, 0].argsort()]
right_center = right_center[right_center[:, 0].argsort()]

bmp_img = np.load(lane_with_angle_adr)
            ...
            if stParam.getStatus() in [0, 5] or stParam.getStatus() in [3, 4]:
                try:
                    ...
                    if stParam.getShowRLFusion():
                        ...
                        # #####*****  查询方式计算目标的航向角  *****#####
                        trackers_cov_old_for_sort = find_angle(trackers_cov_old_for_sort, bmp_img, 1)
    ...
    while True:
        try:
            ...
            for i in range(npFrameTarget.shape[0]):
                ...
                stBoxInfo.nPoints = 0
                stBoxInfo.strClass = switch_send[nType]
