...
class KalmanFilter(object):
    ...
    **Example**
    ...
    def predict_new(self, start_speed, speed, head_angle, acc, u=None, B=None, F=None, Q=None):
        """
        ...
        #     self.x = dot(F, self.x)
        if start_speed:
            dis = speed * 0.1 + 0.5 * 0.1 * 0.1 * acc
            theta = (-1 * head_angle - 90) % 360
            self.x[0][0] += dis * math.cos(theta*np.pi/180)
            self.x[1][0] += dis * math.sin(theta*np.pi/180)
            self.x[4][0] = speed * math.cos(theta*np.pi/180)
            self.x[5][0] = speed * math.sin(theta*np.pi/180)
        else:
            self.x = dot(F, self.x)
    ...
    def update(self, z, reduce, R=None, H=None):
        """
